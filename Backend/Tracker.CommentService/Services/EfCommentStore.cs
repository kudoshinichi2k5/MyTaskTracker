...
public sealed class EfCommentStore
{
    private readonly CommentDbContext _db;

    public EfCommentStore(CommentDbContext db)
    {
        _db = db;
    }

    public IReadOnlyCollection<CommentItem>
        GetForTask(int taskId)
    {
        return _db.Comments
            .AsNoTracking()
    ...
    public CommentItem? Update(
        Guid id,
        string authorUserId,
        string body)
    {
        var comment =
            _db.Comments.FirstOrDefault(
                c => c.Id == id &&
                     c.AuthorUserId ==
                     authorUserId);
